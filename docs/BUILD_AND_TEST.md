...
When a new Eos version changes `.esf2` or `.esf3d`, add a minimal sanitized export under
`Fixtures/EosShowArchive/Compatibility` rather than replacing an old fixture. The Infrastructure build derives its
embedded tested-version catalog from those contracts, and the publish job runs only after the golden-master tests pass.
