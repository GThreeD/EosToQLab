    ...
    string? CueValue)
{
    public static QLabEosNetworkCommand RunCue(
        ...
        string? user = null) =>
        new(
            Type: QLabEosTargetType.Cues,
            SpecifyUser: !string.IsNullOrWhiteSpace(user),
            User: NullIfWhiteSpace(user),
            Command: QLabEosCommand.RunCue,
            ListValue: null,
            CueValue: Required(cue, nameof(cue)));
    ...
    public static QLabEosNetworkCommand RunCueInSpecificList(
        ...
        new(
            ...
            SpecifyUser: !string.IsNullOrWhiteSpace(user),
            User: NullIfWhiteSpace(user),
            Command: QLabEosCommand.RunCueInSpecificList,
            ListValue: Required(list, nameof(list)),
            CueValue: Required(cue, nameof(cue)));
    ...
    public IReadOnlyList<QLabNetworkParameterAssignment> BuildParameters()
    {
        ...
        if (user is not null)
        {
            Add(result, QLabEosParameter.User, user);
        }
        ...
        if (list is not null)
        {
            Add(result, QLabEosParameter.List, list);
        }

        var cue = NullIfWhiteSpace(CueValue);
        if (cue is not null)
        {
            Add(result, QLabEosParameter.Cue, cue);
        }
